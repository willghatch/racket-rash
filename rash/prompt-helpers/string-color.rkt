...
; for style arguments void is treated as keep whatever outer struct style was,
; #f means get rid out outer style
(define (create-styled-struct #:fg [foreground (void)]
                              #:bg [background (void)]
                              #:bold? [bold? (void)]
                              #:italic? [italic? (void)]
                              #:underlined? [underlined? (void)]
...
(define (create-combined-style-hash h1 h2)
  (define (decide-new hash-key)
    (cond
      [(void? (hash-ref h2 hash-key)) (if (hash-ref h2 'reset-before?) ; check if user expects no inherited
                                          #f
  ...
  (hash-set! new-hash 'reset-before? (hash-ref h2 'reset-before?)) ; defaults to #f, not void
...
(define (styled-struct->string ss [outer-style-hash #hash((foreground . #f) ; no outer means no styles
                                                          (background . #f)
                                                          (bold? . #f)
                                                          (italic? . #f)
                                                          (underlined? . #f)
