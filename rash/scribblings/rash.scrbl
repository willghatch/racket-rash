...
Actually, the rc files are found using @racket[list-config-files].  You can read its docs for specifics, but the gist is that it depends on the value of the @tt{XDG_CONFIG_HOME} and @tt{XDG_CONFIG_DIRS} environment variables.  @tt{$HOME/.config/rash/rashrc} is the default on Unix, while the default is @tt{%LOCALAPPDATA%/rash/rashrc} on Windows.
